...
do
tag=mnn_rwf${rejected_weight_floor}_seed${seed}_${init_txt}_pl${paired_length}_bs${bs}_tau${tau}
...
PYTHONHASHSEED=$seed CUDA_VISIBLE_DEVICES=$gpu "$python_cmd" train.py --gpu "$gpu" --seed "$seed" --paired_length "$paired_length" \
  ...
  --memory_update_interval "$memory_update_interval" --rejected_weight_floor "$rejected_weight_floor"
done
